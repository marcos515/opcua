...
class NumericRange {
    constructor(value, secondValue) {
        ...
        node_opcua_assert_1.assert(!secondValue || typeof secondValue === "number" || Array.isArray(secondValue));
    ...
    set_values_matrix(sourceToAlter, newMatrix) {
        ...
        const self = this;
        node_opcua_assert_1.assert(dimensions, "expecting valid dimensions here");
        if (self.type !== NumericRangeType.MatrixRange) {
            // istanbul ignore next
            return { matrix, statusCode: node_opcua_status_code_1.StatusCodes.BadTypeMismatch };
        }
        node_opcua_assert_1.assert(dimensions.length === 2);
        const nbRows = dimensions[0];
        const nbCols = dimensions[1];
        node_opcua_assert_1.assert(sourceToAlter.matrix.length === nbRows * nbCols);
